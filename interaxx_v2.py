...
async def process_target(
    ...
    try:
        # -------------------------
        # IMPORTANT CHANGE:
        # - In --subs mode: inject headers (headers-only)
        # - In --urls mode: inject query params ONLY (no header injection)
        # -------------------------
        if args.subs:
            # Subdomain mode: inject headers only (once per subdomain)
        ...
        async with async_playwright() as p:
            ...
            # Set extra headers on the page context (empty dict ok)
            if req_headers:
...
def parse_args():
    ...
    group.add_argument("-u", "--urls", help="File containing list of URLs (query param testing only)")
